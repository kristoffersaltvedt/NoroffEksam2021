...
public:
    FConsumedCapacity &fromAWS(const Aws::DynamoDB::Model::ConsumedCapacity &awsConsumedCapacity) {
        ...
        for (const std::pair<Aws::String, Aws::DynamoDB::Model::Capacity>& elem : awsConsumedCapacity.GetLocalSecondaryIndexes()) {
            this->localSecondaryIndexes.Add(elem.first.c_str(), FCapacity().fromAWS(elem.second));
        }

        this->globalSecondaryIndexes.Empty();
        for (const std::pair<Aws::String, Aws::DynamoDB::Model::Capacity>& elem : awsConsumedCapacity.GetGlobalSecondaryIndexes()) {
