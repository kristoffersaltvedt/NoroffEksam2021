...
public:
    FTransactWriteItemsResult &fromAWS(const Aws::DynamoDB::Model::TransactWriteItemsResult &awsTransactWriteItemsResult) {
        ...
        for (const std::pair<Aws::String, Aws::Vector<Aws::DynamoDB::Model::ItemCollectionMetrics>>& elem : awsTransactWriteItemsResult.GetItemCollectionMetrics()) {
            ...
            this->itemCollectionMetrics.Add(elem.first.c_str(), Aws::Utils::Json::JsonValue().AsArray(jsonArray).View().WriteReadable().c_str());
        }
